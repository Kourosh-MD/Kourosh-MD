...
def profile_data(username: str) -> dict:
    ...
    code_repos = [repo for repo in original if str(repo.get("name", "")).lower() != username.lower()]
    languages: dict[str, int] = {}
    for repo in code_repos[:20]:
        languages_url = repo.get("languages_url")
        if not languages_url:
            continue
        try:
            breakdown = request_json(languages_url)
        except urllib.error.URLError:
            continue
        if not isinstance(breakdown, dict):
            continue
        for language, byte_count in breakdown.items():
            languages[str(language)] = languages.get(str(language), 0) + int(byte_count)
    return {
...
def portrait_lines(cfg: dict, cols: int = 66, max_rows: int = 50) -> list[str]:
    ascii_path = cfg.get("portrait_ascii")
    if ascii_path:
        path = ROOT / ascii_path
        if not path.exists():
            raise RuntimeError(f"ASCII portrait not found: {path}")
...
def identity_svg(cfg: dict) -> str:
    ...
    if cfg.get("portrait_ascii"):
        max_cols = max(1, max((len(line) for line in portrait), default=1))
        row_count = max(1, len(portrait))
        char_w = min(14.0, 250 / max_cols)
        line_h = min(28.0, 238 / row_count)
        font_size = min(24.0, line_h * .82, char_w * 1.5)
    else:
        char_w, line_h, font_size = 4.0, 5.3, 5.9
    portrait_x, portrait_y = 43.0, 87.0
    if cfg.get("portrait_ascii"):
        portrait_x += max(0, (256 - max_cols * char_w) / 2)
        portrait_y += max(0, (250 - (row_count - 1) * line_h) / 2)
    ...
    skills = "".join(
        f'<g transform="translate({337 + (index % 4) * 130} {313 + (index // 4) * 22})">'
        f'<rect width="120" height="18" rx="9" fill="#0d3636" stroke="#17675f"/>'
        f'<text x="60" y="12.5" text-anchor="middle" class="chip">{escape(skill)}</text></g>'
        for index, skill in enumerate(cfg.get("skills", [])[:8])
    )
...
<text x="337" y="296" class="body">{escape(trim(cfg['role'], 58))} · {escape(cfg['location'])}</text>{skills}
...
def signal_svg(cfg: dict, profile: dict, contribution: dict) -> str:
    ...
    stack_nodes = []
    proficiencies = cfg.get("proficiencies", [])[:8]
    if proficiencies:
        stack_title = "Technology Proficiency"
        stack_label = "SELF-RATED PROFICIENCY / PROFILE CONFIG"
        scan_label = "&gt; PROFICIENCY.SCAN"
        for index, item in enumerate(proficiencies):
            column, row = divmod(index, 4)
            base_x = 42 + column * 403
            y = 363 + row * 35
            percent = max(0, min(100, int(item.get("percent", 0))))
            width = max(10, int(176 * percent / 100))
            color = palette[index % len(palette)]
            stack_nodes.append(f'<circle cx="{base_x + 20}" cy="{y - 4}" r="5" fill="{color}"/><text x="{base_x + 40}" y="{y}" class="language">{escape(str(item.get("name", "")))}</text><text x="{base_x + 182}" y="{y}" text-anchor="end" class="percent" fill="{color}"><tspan fill="{color}">{percent}%</tspan></text><rect x="{base_x + 202}" y="{y - 12}" width="176" height="9" rx="5" fill="#224043"/><rect x="{base_x + 202}" y="{y - 12}" width="{width}" height="9" rx="5" fill="{color}" class="bar" style="animation-delay:{.15 + index * .08:.2f}s"/>')
    else:
        languages = profile.get("languages")
        if languages:
            stack_label = "GITHUB LANGUAGE BYTES / PUBLIC REPOSITORIES"
        else:
            languages = [(skill, 1) for skill in cfg.get("skills", [])[:5]]
            stack_label = "DECLARED TECHNOLOGIES / EQUAL DISPLAY WEIGHT"
        stack_title = "Language Stack"
        scan_label = "&gt; STACK.SCAN"
        ...
        for index, (language, count) in enumerate(languages[:5]):
            ...
            color = palette[index]
            stack_nodes.append(f'<circle cx="62" cy="{y - 4}" r="5" fill="{color}"/><text x="82" y="{y}" class="language">{escape(language)}</text><text x="285" y="{y}" text-anchor="end" class="percent" fill="{color}"><tspan fill="{color}">{percent}%</tspan></text><rect x="315" y="{y - 12}" width="510" height="9" rx="5" fill="#224043"/><rect x="315" y="{y - 12}" width="{width}" height="9" rx="5" fill="{color}" class="bar" style="animation-delay:{.15 + index * .08:.2f}s"/>')
...
<rect x="6" y="258" width="888" height="256" rx="20" fill="url(#shell)" stroke="{HAIR}"/><rect x="24" y="276" width="852" height="220" rx="18" fill="{PANEL}" stroke="#1ca596"/><text x="43" y="312" class="title">{stack_title}</text><text x="43" y="330" class="label">{stack_label}</text><text x="849" y="312" text-anchor="end" class="label">{scan_label}</text>{''.join(stack_nodes)}
