...
def generate(config: dict) -> str:
    ...
    summary = list(resume.get("summary", []))[:3]
    education = resume.get("education", {})
    stack = list(resume.get("core_stack", []))[:6]
    languages = list(resume.get("languages", []))[:3]
    colors = [TEAL, BLUE, PURPLE, YELLOW, GREEN, ORANGE]

    summary_nodes = "".join(
        f'<text x="67" y="{190 + index * 30}" class="copy">{clean(line, 73)}</text>'
        for index, line in enumerate(summary)
    )
    stack_nodes = "".join(
        f'<g class="skill skill-{index}" transform="translate({66 + (index % 3) * 154} {409 + (index // 3) * 48})">'
    ...
    language_nodes = "".join(
        f'<g class="language-row row-{index}" transform="translate(602 {408 + index * 56})">'
...
.panel-1{animation-delay:.12s}.panel-2{animation-delay:.26s}.panel-3{animation-delay:.4s}.panel-4{animation-delay:.54s}
...
.skill-3{animation-delay:.85s}.skill-4{animation-delay:.95s}.skill-5{animation-delay:1.05s}
...
@keyframes scan{from{transform:translateY(-40px)}to{transform:translateY(600px)}}
@media(prefers-reduced-motion:reduce){*{animation:none!important}.panel,.skill,.language-row{opacity:1!important}}
"""

    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="900" height="550" viewBox="0 0 900 550" role="img" aria-label="Animated resume for {clean(config.get('name', 'Kourosh Mansouri'))}">
<defs>
  ...
  <clipPath id="resume-clip"><rect x="18" y="18" width="864" height="514" rx="18"/></clipPath>
  <style>{css}</style>
</defs>
<rect width="900" height="550" rx="24" fill="{OUTER}"/>
<rect x="6" y="6" width="888" height="538" rx="20" fill="url(#resume-shell)" stroke="{HAIR}"/>
<rect x="18" y="18" width="864" height="514" rx="18" fill="{BG}" stroke="#1a7269"/>
...
<g class="panel panel-1"><rect x="44" y="147" width="510" height="177" rx="18" fill="{PANEL}" stroke="#1c8277"/><text x="66" y="176" class="section">Professional Summary</text><text x="526" y="176" text-anchor="end" class="label">PROFILE.INIT</text>{summary_nodes}</g>
<g class="panel panel-2"><rect x="574" y="147" width="282" height="177" rx="18" fill="{PANEL}" stroke="#1c8277"/><text x="596" y="176" class="section">Education</text><text x="832" y="176" text-anchor="end" class="label">ACTIVE</text><circle cx="603" cy="211" r="7" fill="{MINT}"/><path d="M603 218V278" stroke="#26786f" stroke-width="2"/><text x="625" y="215" class="education-main">{clean(education.get('program', 'Computer Science'), 31)}</text><text x="625" y="243" class="education-sub">{clean(education.get('institution', 'Islamic Azad University'), 34)}</text><text x="625" y="266" class="education-sub">{clean(education.get('location', config.get('location', '')), 34)}</text><text x="625" y="294" class="label">CURRENTLY STUDYING</text></g>
<g class="panel panel-3"><rect x="44" y="342" width="510" height="166" rx="18" fill="{PANEL}" stroke="#1c8277"/><text x="66" y="375" class="section">Java-Focused Stack</text><text x="526" y="375" text-anchor="end" class="label">CORE.TOOLKIT</text>{stack_nodes}</g>
<g class="panel panel-4"><rect x="574" y="342" width="282" height="166" rx="18" fill="{PANEL}" stroke="#1c8277"/><text x="596" y="375" class="section">Languages</text><text x="832" y="375" text-anchor="end" class="label">COMMUNICATION</text>{language_nodes}</g>
<g clip-path="url(#resume-clip)" opacity=".55"><rect x="18" y="0" width="864" height="2" fill="url(#scan-line)" class="scan"/></g>
<text x="849" y="527" text-anchor="end" class="mono">{clean(config.get('status', 'Coding · Learning'), 62)}</text>
</svg>'''
